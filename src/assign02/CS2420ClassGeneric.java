...
public class CS2420ClassGeneric<Type> {
	 ...
	 *
	 * @param student of <Type> - student to be added to this CS 2420 class
	 ...
	 * Only expects a CS2420 Student regardless of type as it pulls from the studentList of <Type>
	 */
	 ...
	 *
	 * @param contactInfo of <Type> - contact information of student(s) to be retrieved
	 * @return a list of the CS 2420 student(s) of <Type> with the given contact information (in any order),
	...
	public ArrayList<CS2420StudentGeneric<Type>> lookup(Type contactInfo) {
		...
		ArrayList<CS2420StudentGeneric<Type>> sameContact = new ArrayList<CS2420StudentGeneric<Type>>();
		for (CS2420StudentGeneric s : studentList) {
	...
	public void addScore(int uNID, double score, String category) {
		...
		CS2420StudentGeneric<Type> targetStudent = lookup(uNID);

		// If there is no student with such uNID
		if (targetStudent == null) {
			return;
		}

		//assigns score to category in assign02.CS2420Student object; has no effect if not included category
		targetStudent.addScore(score, category);
	}
	...
	public double computeClassAverage() {
		...
		//finds total score of class
		double classSum = 0;
		for (CS2420StudentGeneric s : studentList) {
			classSum += s.computeFinalScore();
		}
	...
	public ArrayList<Type> getContactList() {
		...
		ArrayList<Type> dupesContactList = new ArrayList<Type>();
		ArrayList<Type> noDupesContactList = new ArrayList<Type>();
		for (CS2420StudentGeneric s : studentList) {
			dupesContactList.add((Type)s.getContactInfo());
		}
		boolean noDupes;
		for (Type e : dupesContactList) {
			noDupes = true;
			for (Type a : noDupesContactList) {
				if (e.equals(a)) {
					noDupes = false;
					break;
				}
			...
			if (noDupes) {
				noDupesContactList.add(e);
			}
		}
		return noDupesContactList;
	}
	...
	protected class OrderByName implements Comparator<CS2420StudentGeneric<Type>> {
		// compares last names, then first names, and finally uNID's through nested if else statements
		// returning either a negative value if the left hand side is less than rhs, or positive if vice versa
		// zeros are not expected in this comparator as uNID's are unique

		public int compare(CS2420StudentGeneric<Type> lhs, CS2420StudentGeneric<Type> rhs){
			String lhsLastName=lhs.getLastName();
			String lhsFirstName=lhs.getFirstName();
			String rhsLastName=rhs.getLastName();
			String rhsFirstName=rhs.getFirstName();
			int lastNameComp = lhsLastName.compareTo(rhsLastName);
			if (lastNameComp<0){
				return lastNameComp;
			}
				...
				int firstNameComp = lhsFirstName.compareTo(rhsFirstName);
				if (firstNameComp<0){
	...
	protected class OrderByScore implements Comparator<CS2420StudentGeneric<Type>> {
		// Compares final scores, however returns positive 1 when the lhs is LESS than rhs as it is sorted in descending order
		// In the event of a tie, the sorting then resorts to uNID's and sorting them in ascending order
